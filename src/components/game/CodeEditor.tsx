    ...
    <div className="w-1/2 space-y-4 flex flex-col max-h-[calc(100vh-100px)] overflow-hidden">
      <div className="bg-[#131324] p-4 rounded-lg flex-1 flex flex-col overflow-hidden">
        <div className="flex justify-between items-center mb-2">
          <div className="text-cyan-400 font-medium text-base">
            {"</>"} Code
          </div>
          <select
      ...
      {result && (result.output || result.error) && (
        <div className="bg-[#101828] text-white mt-4 p-4 rounded-lg max-h-96 overflow-auto border border-cyan-400">
          <div
            className={`font-semibold w-fit px-3 py-1 rounded text-white mb-2 ${result.success ? "bg-green-400" : "bg-red-500"}`}
          >
            {result.success ? "Output:" : "Error:"}
          </div>
          <pre
            className={`whitespace-pre-wrap text-sm ${result.success ? "text-green-400" : "text-red-500"} font-bold`}
          >
